...
//CODE HERE
// let mappedArr = customers.map( customer => customer.email);
mappedArr = _.map(customers, 'email');
mappedArr
...
let fullList = [...inviteList1, ...inviteList2.filter( name => !inviteList1.includes(name))]
// let fullList = _.union(inviteList1,inviteList2)
fullList
...
// let list = [...friendsOfJim.filter( name => friendsOfBetty.includes(name))];
let list = _.intersection(friendsOfJim, friendsOfBetty)
list
...
let purchasesByCompany = {"Staples": purchases.filter( purchase => purchase.company === "Staples"),"Dunder Mifflin": purchases.filter( purchase => purchase.company === "Dunder Mifflin")};
// let purchasesByCompany = _.groupBy(purchases, 'company')
purchasesByCompany
